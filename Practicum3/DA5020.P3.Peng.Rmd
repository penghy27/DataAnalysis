...
The dataset includes logical data for "ehai_fee," although "ehai_fee" has more than a 90% missing value. "total_amount" and "trip_type" have fewer missing values, and we will address these missing values later.


###1.1 Tranform data type
```{r}
...
After convert some variables to their data type, we found some issues in this data set.

(a) Since the dataset contains numerical variables related to taxes, tips, charges, etc., the numbers should be positive or zero. However, some variables have minimum values that are negative, which should be unreasonable. These variables include “fare_amount,” “extra,” “mta_tax,” “tip_amount,” “improve- ment_surcharge,” and “total_amount.” 
...
ggplot(tripdata_df, aes(x = pickup)) +
  ...
  labs(x = "Dropoff Date", y = "Frequency", title = "Distribution of Pickup Date") +
  facet_wrap(~year(lpep_pickup_datetime), scales = "free") +
...
ggplot(tripdata_df, aes(x = dropoff)) +
  ...
  facet_wrap(~year(lpep_dropoff_datetime), scales = "free") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
tripdata_df$pickup_weekday <- weekdays(tripdata_df$lpep_pickup_datetime)

# Convert dropoff date to weekday 
tripdata_df$dropoff_weekday <- weekdays(tripdata_df$lpep_dropoff_datetime)
...
- **Trip_distance** and **total_amount** and **tolls_amount** might be more related to tip_amount
...
# Convert categorical variables to factors
filtered_df[cat_var] <- lapply(filtered_df[cat_var], as.factor)
...
for (var in cat_var) {
  # Create a table of frequencies
  table_data <- table(filtered_df[[var]])
...
### 1.3.3 Catgorical var cleaning, Transform one-hot encoding
```{r}
...
normalized_subset <- data.frame(lapply(normalized_df[, 1:8], min_max_normalize))

normalized_df <- data.frame(normalized_df, tip_amount = combined_data$tip_amount)

normalized_df <- normalized_df %>% 
  select(-c(combined_data.tip_amount, tip_amount.1, tip_amount.2))
...
index <- createDataPartition(new_data$tip_amount, p = 0.8, list = FALSE) # Create training set
...
knn.predict <- function(data_train, data_test, k) {
  ...
  knn_model <- FNN::knn.reg(train = predictors_train, 
